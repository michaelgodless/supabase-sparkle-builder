-- Исправление функций триггеров аудита для правильного приведения типов
...
BEGIN
  IF (TG_OP = 'UPDATE') THEN
    INSERT INTO public.audit_logs (
      ...
      TG_OP::action_type, 'viewings', NEW.id, auth.uid(), to_jsonb(OLD), to_jsonb(NEW)
    );
    RETURN NEW;
  ELSIF (TG_OP = 'INSERT') THEN
    INSERT INTO public.audit_logs (
      ...
      TG_OP::action_type, 'viewings', NEW.id, auth.uid(), to_jsonb(NEW)
    );
    RETURN NEW;
  ELSIF (TG_OP = 'DELETE') THEN
    INSERT INTO public.audit_logs (
      ...
      TG_OP::action_type, 'viewings', OLD.id, auth.uid(), to_jsonb(OLD)
    );
    RETURN OLD;
  END IF;
END;
